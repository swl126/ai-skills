# Agent Permission Engine executable contract
...
The engine requires evidence records for these categories: `tools`, `credentials`, `data`, `network`, `approvals`, `delegation`. Domain thresholds live in `engine-profile.json`, and the input shape is defined by `../schemas/assessment.schema.json`.
