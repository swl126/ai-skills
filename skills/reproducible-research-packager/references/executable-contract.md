# Research Reproducibility Engine executable contract
...
The engine requires evidence records for these categories: `inputs`, `code`, `environment`, `execution`, `checksums`, `licensing`, `reconstruction`. Domain thresholds live in `engine-profile.json`, and the input shape is defined by `../schemas/assessment.schema.json`.
