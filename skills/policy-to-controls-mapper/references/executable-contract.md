# Policy Control Mapping Engine executable contract
...
The engine requires evidence records for these categories: `requirements`, `controls`, `owners`, `evidence`, `tests`, `exceptions`. Domain thresholds live in `engine-profile.json`, and the input shape is defined by `../schemas/assessment.schema.json`.
