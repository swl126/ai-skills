# Dependency risk register — worked example

## Decision summary

- **Decision:** PATCH NOW: the parser is reachable from unauthenticated uploads; isolate and replace the CI-only plugin.
- **Scope and artifact identity:** EXAMPLE-dependency-risk-auditor-v1
...
| --- | --- | --- | --- | --- |
| E-1 | Sanitized scenario fixture v1 | Assess a web service with a critical transitive advisory and an abandoned build plugin. | Frozen example input | Does not represent a live system |
...
| --- | --- | --- | --- | --- | --- | --- |
| F-1 | PATCH NOW: the parser is reachable from unauthenticated uploads; isolate and replace the CI-only plugin. | medium | E-1, E-2 | Resolve the named condition and rerun the same acceptance check | Example owner | Before promotion |
