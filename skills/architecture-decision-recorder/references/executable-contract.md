# Architecture Decision Engine executable contract
...
The engine requires evidence records for these categories: `context`, `options`, `criteria`, `decision`, `consequences`, `reversal`. Domain thresholds live in `engine-profile.json`, and the input shape is defined by `../schemas/assessment.schema.json`.
