# Disaster Recovery Engine executable contract
...
The engine requires evidence records for these categories: `scope`, `safety`, `restore`, `validation`, `rto`, `rpo`, `retest`. Domain thresholds live in `engine-profile.json`, and the input shape is defined by `../schemas/assessment.schema.json`.
