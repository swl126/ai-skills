# Database Migration Engine executable contract
...
The engine requires evidence records for these categories: `expand`, `backfill`, `compatibility`, `rehearsal`, `recovery`, `contract`. Domain thresholds live in `engine-profile.json`, and the input shape is defined by `../schemas/assessment.schema.json`.
