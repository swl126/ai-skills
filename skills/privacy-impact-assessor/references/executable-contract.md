# Privacy Impact Engine executable contract
...
The engine requires evidence records for these categories: `collection`, `purpose`, `minimization`, `retention`, `sharing`, `rights`, `risk`. Domain thresholds live in `engine-profile.json`, and the input shape is defined by `../schemas/assessment.schema.json`.
