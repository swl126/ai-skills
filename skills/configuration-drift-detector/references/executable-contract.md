# Configuration Drift Engine executable contract
...
The engine requires evidence records for these categories: `identity`, `logging`, `encryption`, `network`, `application`. Domain thresholds live in `engine-profile.json`, and the input shape is defined by `../schemas/assessment.schema.json`.
