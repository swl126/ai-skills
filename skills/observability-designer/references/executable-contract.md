# Observability Design Engine executable contract
...
The engine requires evidence records for these categories: `slis`, `logs`, `metrics`, `traces`, `alerts`, `privacy`, `cost`. Domain thresholds live in `engine-profile.json`, and the input shape is defined by `../schemas/assessment.schema.json`.
