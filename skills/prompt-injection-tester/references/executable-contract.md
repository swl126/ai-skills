# Prompt Injection Result Engine executable contract
...
The engine requires evidence records for these categories: `direct`, `indirect`, `retrieval`, `tools`, `secrets`, `approvals`. Domain thresholds live in `engine-profile.json`, and the input shape is defined by `../schemas/assessment.schema.json`.
