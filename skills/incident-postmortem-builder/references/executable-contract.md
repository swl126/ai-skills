# Incident Postmortem Engine executable contract
...
The engine requires evidence records for these categories: `timeline`, `impact`, `causality`, `detection`, `recovery`, `actions`. Domain thresholds live in `engine-profile.json`, and the input shape is defined by `../schemas/assessment.schema.json`.
