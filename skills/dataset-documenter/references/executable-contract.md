# Dataset Documentation Engine executable contract
...
The engine requires evidence records for these categories: `provenance`, `population`, `schema`, `transformations`, `limitations`, `governance`. Domain thresholds live in `engine-profile.json`, and the input shape is defined by `../schemas/assessment.schema.json`.
