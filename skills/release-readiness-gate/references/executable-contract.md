# Release Readiness Engine executable contract
...
The engine requires evidence records for these categories: `tests`, `security`, `rollback`, `monitoring`, `migration`, `documentation`. Domain thresholds live in `engine-profile.json`, and the input shape is defined by `../schemas/assessment.schema.json`.
